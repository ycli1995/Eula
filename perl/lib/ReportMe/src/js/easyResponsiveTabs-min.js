  ...
  a.fn.extend({
    easyResponsiveTabs: function (k) {
      ...
      this.each(function () {
        ...
        var s = p.attr("id");
        p.find("ul.resp-tabs-list." + k.tabidentify + " li")
          .addClass("resp-tab-item")
          .addClass(k.tabidentify);
        p.css({ display: "block", width: i });
        if (k.type == "vertical") {
          t.css("margin-top", "3px");
        }
        p.find(".resp-tabs-container." + k.tabidentify).css(
          "border-color",
          k.active_content_border_color,
        );
        p.find(".resp-tabs-container." + k.tabidentify + " > div")
        ...
        var m;
        p.find(".resp-tab-content." + k.tabidentify).before(
          "<h2 class='resp-accordion " +
            k.tabidentify +
            "' role='tab'><span class='resp-arrow'></span></h2>",
        );
        p.find(".resp-tab-content." + k.tabidentify)
          .prev("h2")
          .css({
        ...
        a(p.find(".resp-tab-item." + k.tabidentify)[n])
        ...
        if (
          k.closed !== true &&
          !(k.closed === "accordion" && !t.is(":visible")) &&
          !(k.closed === "tabs" && t.is(":visible"))
        ) {
          a(p.find(".resp-accordion." + k.tabidentify)[n])
            .addClass("resp-tab-active")
            .css({
          ...
          a(p.find(".resp-tab-content." + k.tabidentify)[n])
        ...
        p.find("[role=tab]").each(function () {
          ...
          v.click(function () {
            ...
            if (B.hasClass("resp-accordion") && B.hasClass("resp-tab-active")) {
              p.find(".resp-tab-content-active." + k.tabidentify).slideUp(
                "",
                function () {
            ...
            if (
              !B.hasClass("resp-tab-active") &&
              B.hasClass("resp-accordion")
            ) {
              p.find(".resp-tab-active." + k.tabidentify)
                .removeClass("resp-tab-active")
                .css({
                  "background-color": k.inactive_bg,
                  "border-color": "none",
                });
              p.find(".resp-tab-content-active." + k.tabidentify)
                .slideUp()
                .removeClass("resp-tab-content-active resp-accordion-closed");
              p.find("[aria-controls=" + w + "]")
                .addClass("resp-tab-active")
                .css({
                  "background-color": k.activetab_bg,
                  "border-color": k.active_border_color,
